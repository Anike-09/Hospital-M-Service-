package com.HPS.constant;

public class Constant {
	
	public static final Object OTP_SENDCESS = "OTP send successful...";
	public static final String PASSWORD_PATTERN = "^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9!@#$%^&*()_+\\-=\\[\\]{};':\"\\\\|,.<>\\/?]).{8,}$";
	public static final String SOMETHING_WdfdfdENT_WadfsdfRONG = "Something is wrong ";
	public static final String DELETE_SUfgjhgCCESS = "Delete Successfully";
	public static final String SOME_IS_WRONG = "Something is wrong";
	public static final int MAX_OPT_HOLIDAYS_PER_YEAR = 5;
	
	public static final int MAX_OPTIONAL_HOLIDAYS_PER = 5;
	
	public static final int MAX_OPTIONAL_HOLIDAYS_PERYEAR = 5;
	public static final int MOPTIONAL_HOLIDAYER_YEAR = 5;
	public static final int MAX_OPTIONAL_HOLIDAYER_YEAR = 5;
	public static final int gdghfghfgh = 5;
	public static final int M_OPTgIONAL_HOLDAYER_YEAR = 5;
	public static final int MAX_OPTIONAL_HOLDAYER_YEAR = 5;
	public static final int MAX_OTIONAL_HOLIDAYER_YEAR = 5;
	public static final int MA_OPTIONAL_HOLDAYER_YEAR = 5;
}
